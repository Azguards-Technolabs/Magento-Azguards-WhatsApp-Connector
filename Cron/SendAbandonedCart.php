...
class SendAbandonedCart
{
    ...
    public function execute()
    {
        try {
            ...
            // $response = $this->apiHelper->sendMessage($abandonCartTempaletId, $abandonCartTempaletVerible, 'SendAbandonedCart');
        } catch (\Exception $e) {
    ...
    protected function getAbandonedCarts()
    {
        ...
        $collection = $this->quoteCollectionFactory->create()->addFieldToFilter('is_active', 1)->addFieldToFilter('items_count', ['gt' => 0])->addFieldToFilter('updated_at', ['lt' => date('Y-m-d H:i:s', strtotime("-{$minutes} minutes"))]);
        return $collection;
    }
