...
class MediaPersistenceService
{
    ...
    public function persistFromUrl(string $url, string $fileName): ?string
    {
        ...
        try {
            ...
            $this->curl->setHeaders([]); // CRITICAL: Clear any existing headers (like Bearer tokens) that S3 rejects
            ...
            $this->logger->info("MediaPersistence: Curl completed. Status: $status, Content length: " . strlen((string)$content));

            if ($status !== 200 || empty($content)) {
                $this->logger->warning("MediaPersistence: Failed to download $fileName. Status: $status. Error Body: " . substr((string)$content, 0, 500));
                return null;
            }
